...
export class AuthRepository{
 ...
 repairPartialAccount(v:{id:string;hash:string;salt:string;params:string;sessionId:string;tokenHash:string;now:number}){return this.db.batch([
  {sql:'INSERT INTO user_credentials(user_id,password_hash,password_salt,kdf,kdf_params,updated_at) VALUES(?,?,?,?,?,?)',args:[v.id,v.hash,v.salt,'PBKDF2-SHA256',v.params,v.now]},
  {sql:'INSERT INTO sessions(id,user_id,token_hash,created_at,expires_at) VALUES(?,?,?,?,?)',args:[v.sessionId,v.id,v.tokenHash,v.now,v.now+2592000000]},
  {sql:'INSERT INTO audit_events(id,actor_user_id,event_type,object_type,object_id,payload_json,created_at) VALUES(?,?,?,?,?,?,?)',args:[crypto.randomUUID(),v.id,'ACCOUNT_REGISTRATION_REPAIRED','USER',v.id,'{}',v.now]}
 ]);}
