...
export class AdminToolsRepository{
  ...
  evidence(limit=100){return this.db.execute<{id:string;title:string}>('SELECT id,title FROM evidence ORDER BY created_at DESC,id DESC LIMIT ?',[Math.min(100,Math.max(1,limit))]);}
  sources(limit=100){return this.db.execute<{id:string;title:string}>('SELECT id,title FROM sources ORDER BY created_at DESC,id DESC LIMIT ?',[Math.min(100,Math.max(1,limit))]);}
}
