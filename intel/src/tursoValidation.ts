...
export async function validateTurso(c:RequestContext){
  ...
  try{
    ...
    await db.batch([
      {sql:'INSERT INTO investigations(id,slug,title,summary,status,author_user_id,created_at,updated_at,published_at) VALUES(?,?,?,?,?,?,?,?,?)',args:[inv,`validation-${runId}`,`Validation ${runId}`,`INTEL V3 Turso validation ${runId}`,'ACTIVE',null,now,now,now]},
      {sql:'INSERT INTO entities(id,type,canonical_name,normalized_name,description,created_at,updated_at) VALUES(?,?,?,?,?,?,?)',args:[entity,'ORGANIZATION',`Validation Entity ${runId}`,`validation entity ${runId}`,`Graph validation ${runId}`,now,now]},
      {sql:'INSERT INTO entity_aliases(id,entity_id,alias,normalized_alias) VALUES(?,?,?,?)',args:[crypto.randomUUID(),entity,alias,alias.toLowerCase()]}
