...
export class FanoutRepository{
 ...
 async drain(maxBatches=5,batchSize=100){
  ...
  for(let batch=0;batch<Math.min(10,Math.max(1,maxBatches));batch++){
   const now=Date.now(),candidate=await this.db.first<any>("SELECT id FROM notification_fanout_jobs WHERE status<>'DONE' AND (lease_expires_at IS NULL OR lease_expires_at<?) ORDER BY created_at,id LIMIT 1",[now]);
   if(!candidate)break;
   await this.db.execute("UPDATE notification_fanout_jobs SET status='RUNNING',lease_owner=?,lease_expires_at=?,updated_at=? WHERE id=? AND status<>'DONE' AND (lease_expires_at IS NULL OR lease_expires_at<?)",[owner,now+60_000,now,candidate.id,now]);
