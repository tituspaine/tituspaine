...
if (location.pathname.startsWith('/investigations/')||location.pathname.startsWith('/evidence/')||location.pathname.startsWith('/entities/')) {
  ...
  if(h){
    const b=document.createElement('button'); b.type='button'; b.textContent='SHARE'; b.className='blue'; b.style.marginBottom='20px';
    b.addEventListener('click',async()=>{try{if(navigator.share)await navigator.share({title:document.title,url:location.href});else await navigator.clipboard.writeText(location.href);}catch{}});
